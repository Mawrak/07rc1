...
u32	vertex_in_direction(u32 level_vertex_id, Fvector direction, float max_distance)
{
	...
	u32				_result = u32(-1);
	ai().level_graph().farthest_vertex_in_direction(level_vertex_id,start_position,finish_position, _result,0);
	return			(ai().level_graph().valid_vertex_id(_result) ? _result : level_vertex_id);
}
...
CEnvDescriptor *current_environment(CEnvironment * _self)
{
	return		(&_self->CurrentEnv);
}
