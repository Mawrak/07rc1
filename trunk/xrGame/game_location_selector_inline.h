...
TEMPLATE_SPECIALIZATION
IC	void CGameLocationSelector::select_location	(const _vertex_id_type start_vertex_id, _vertex_id_type &_dest_vertex_id)
{
	switch (m_selection_type) {
		...
		case eSelectionTypeRandomBranching : {
			if (m_graph)
				select_random_location(start_vertex_id, _dest_vertex_id);
			m_failed			= m_failed && (start_vertex_id == _dest_vertex_id);
			break;
		}
		default :				NODEFAULT;
	}
...
TEMPLATE_SPECIALIZATION
IC	void CGameLocationSelector::select_random_location(const _vertex_id_type start_vertex_id, _vertex_id_type &_dest_vertex_id)
{
	VERIFY						(m_graph);
	...
	if (!branch_factor) {
		if ((start_vertex_id != m_previous_vertex_id) && accessible(m_previous_vertex_id))
			_dest_vertex_id		= m_previous_vertex_id;
		else
			_dest_vertex_id		= start_vertex_id;
	}
	else {
