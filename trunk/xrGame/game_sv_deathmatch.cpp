...
//		Msg("------------- Player wants %d", ItemID);
	...
	if (pActor)
	{
		...
		{
			NET_Packet			_P;
			u_EventGen			(_P,GE_DESTROY,*IDI);
			Level().Send(_P,net_flags(TRUE,TRUE));
		};
	};
...
BOOL game_sv_Deathmatch::Is_Anomaly_InLists(CSE_Abstract* E)
{
	...
	for (u32 j=0; j<m_AnomalySetsList.size(); j++)
	{
		...
		ANOMALIES_it _It = std::find(Anomalies->begin(), Anomalies->end(),E->name_replace());
		if (_It != Anomalies->end())
		{
			return TRUE;
		};
	};
