...
void GetCDKey_FromRegistry(char* CDKeyStr)
{
	ReadRegistry_StrValue(REGISTRY_VALUE_GSCDKEY, CDKeyStr);
	if (xr_strlen(CDKeyStr) > 64)
	{
		CDKeyStr[64] = 0;
	}
}

void WriteCDKey_ToRegistry(char* CDKeyStr)
{
	if ( xr_strlen(CDKeyStr) > 64 )
	{
		CDKeyStr[64] = 0;
	}
	WriteRegistry_StrValue(REGISTRY_VALUE_GSCDKEY, CDKeyStr);
}
