...
CConsole::~CConsole()
{
	...
	ConsoleShader.destroy();
	ConsoleShaderG.destroy();
}
...
void CConsole::OnRender	()
{
	...
	if (!pFont)
	{
		pFont = xr_new<CGameFont>("hud_font_di", CGameFont::fsDeviceIndependent, false);
	...
	if(!ConsoleShader)
	{
		ConsoleShader.create("hud\\default", "ui\\ui_console");
	}
	if (!ConsoleShaderG)
	{
		ConsoleShaderG.create(FVF::F_TL, RCache.Vertex.Buffer(), RCache.QuadIB);
	}
	bool bGame = false;
...
void CConsole::DrawBackgrounds( bool bGame )
{
	...
	FVF::TL* pv_start = (FVF::TL*)(RCache.Vertex.Lock(6 + 12 + (VIEW_TIPS_COUNT + 1) * 6 + 12, ConsoleShaderG.stride(), dwOffset));
	FVF::TL* pv = pv_start;
	...
	if ( m_tips.size() == 0 || m_disable_tips )
	{
		...
		p_cnt = pv - pv_start;
		RCache.Vertex.Unlock(u32(p_cnt), ConsoleShaderG.stride());
		dwCount = u32(p_cnt / 3);
		RCache.set_Geometry(ConsoleShaderG);
		RCache.set_Shader(ConsoleShader);
		if (dwCount > 0)
			RCache.Render(D3DPT_TRIANGLELIST, dwOffset, dwCount);
		return;
	}
	...
	if ( m_select_tip < (int)m_tips.size() )
	{
		...
		for ( u32 i = 0; itb1 != ite1; ++itb1, ++i ) // tips
		{
			...
			r1.x1 = pr.x1 + w1 + pFont->SizeOf_( tmp );
			r1.y1 = pr.y1 + i * font_h;
	
			strncpy_s( tmp, str_size+1, ts.text.c_str(), ts.HL_finish );
			r1.x2 = pr.x1 + w1 + pFont->SizeOf_( tmp );
			r1.y2 = r1.y1 + font_h;
	
			DrawRect( r1, tips_word_color, pv );
...
{
	pv->set(r.x1, r.y1, color, 0.f, 0.f); pv++;
	pv->set(r.x2, r.y1, color, 1.f, 0.f); pv++;
	pv->set(r.x2, r.y2, color, 1.f, 1.f); pv++;

	pv->set(r.x1, r.y1, color, 0.f, 0.f); pv++;
	pv->set(r.x2, r.y2, color, 1.f, 1.f); pv++;
	pv->set(r.x1, r.y2, color, 0.f, 1.f); pv++;
