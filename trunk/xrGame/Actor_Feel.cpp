...
void	CActor::PickupModeUpdate_COD	()
{
	...
	{
		ISpatial* _spatial	= ISpatialResult[o_it];
		CInventoryItem*	pIItem	= smart_cast<CInventoryItem*> (_spatial->dcast_CObject        ());
		...
		CGrenade*	pGrenade	= smart_cast<CGrenade*> (_spatial->dcast_CObject        ());
		if (pGrenade && !pGrenade->Useful()) continue;

		CMissile*	pMissile	= smart_cast<CMissile*> (_spatial->dcast_CObject        ());
	...
	{
		CFrustum					_frustum;
		_frustum.CreateFromMatrix	(Device.mFullTransform,FRUSTUM_P_LRTB|FRUSTUM_P_FAR);
		if (!CanPickItem(_frustum,Device.vCameraPosition,&pNearestItem->object()))
			pNearestItem = NULL;
	}
