...
#include "tss.h"
#include "../layers/blenders/blender.h"
#include "../layers/blenders/blender_recorder.h"
	...
	else
	{
		...
		if (SUCCEEDED(_hr))
		{
			if (pShaderBuf)
			{
				...
				if (SUCCEEDED(_hr))	{
					LPCVOID			_data		= NULL;
					_hr	= D3DXFindShaderComment	((DWORD*)pShaderBuf->GetBufferPointer(),MAKEFOURCC('C','T','A','B'),&_data,NULL);
					if (SUCCEEDED(_hr) && _data)
					{
						pConstants				= LPD3DXSHADER_CONSTANTTABLE(_data);
