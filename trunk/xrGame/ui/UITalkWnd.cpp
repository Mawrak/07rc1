...
void CUITalkWnd::UpdateQuestions()
{
	...
	if (!m_pCurrentDialog)
	{
		...
		for (u32 i = 0; i < m_pOurDialogManager->AvailableDialogs().size(); ++i, ++number)
		{
			...
			AddQuestion(phrase_dialog->DialogCaption(), phrase_dialog->GetDialogID(), number, phInfo);
		}
	}
	else
	{
		if (m_pCurrentDialog->IsWeSpeaking(m_pOurDialogManager))
		{
			...
			if (m_pCurrentDialog && !m_pCurrentDialog->allIsDummy())
			{
				...
				for (PHRASE_VECTOR::const_iterator it = m_pCurrentDialog->PhraseList().begin(); it != m_pCurrentDialog->PhraseList().end(); ++it, ++number)
				{
					...
					AddQuestion(phrase->GetText(), phrase->GetID(), number, phInfo);
				}
			}
			else
				UpdateQuestions();
...
void CUITalkWnd::AddQuestion(const shared_str& text, const shared_str& value, int number, SPhraseInfo phInfo)
{
	if (text.size() == 0)
		return;
	UITalkDialogWnd->AddQuestion(*CStringTable().translate(text), value.c_str(), number, phInfo);
}
