...
bool CMissile::TryPlayAnimIdle()
{
	...
	if (pActor)
	{
		...
		pActor->g_State(st);
		if (st.bSprint && m_sAnimIdleSpr != nullptr)
		{
			m_pHUD->animPlay(m_pHUD->animGet(*m_sAnimIdleSpr), TRUE, NULL, GetState());
			return true;
		}
		else
		{
...
void CMissile::activate_physic_shell()
{
	...
	if (entity_alive && entity_alive->character_physics_support()){
		Fvector			_parent_vel;
		entity_alive->character_physics_support()->movement()->GetCharacterVelocity(_parent_vel);
		l_vel.add		(_parent_vel);
	}
