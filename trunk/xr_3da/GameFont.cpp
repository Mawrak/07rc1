...
void CGameFont::Initialize		(LPCSTR cShader, LPCSTR cTextureName, bool need_prefix)
{
	...
	nNumChars = 0x100; TCMap = (Fvector*)xr_realloc((void*)TCMap, nNumChars * sizeof(Fvector));
	...
	else if (ini->section_exist("char_widths"))
	{
		fHeight = ini->r_float("char_widths", "height");
		int cpl = 16;
		for (u32 i = 0; i < nNumChars; i++)
		{
			sprintf_s(buf, sizeof(buf), "%d", i);
			float w = ini->r_float("char_widths", buf);
...
void CGameFont::OnRender()
{
	...
	float angleRad = GetRotation();
	bool is_using_rotation = !fis_zero(angleRad);
	...
	for (u32 i = 0; i < strings.size(); )
	{
		...
		for (; i < last; i++)
		{
			...
			if (len)
			{
				...
				// **Центр вращения = левый верхний угол**
				float centerX = X;
				float centerY = Y;

				// Лямбда-функция для поворота точки
				...
				for (int j = 0; j < len; j++)
				{
					...
					if (!fis_zero(l.z))
					{
						...
						float X1 = X, Y1 = Y2;
						float X2 = X, Y2_ = Y;
						float X3 = X + scw, Y3 = Y2;
						float X4 = X + scw, Y4 = Y;

						// **Применяем поворот, если необходимо**
						if (is_using_rotation)
						{
							RotatePoint(X1, Y1);
							RotatePoint(X2, Y2_);
							RotatePoint(X3, Y3);
						...
						v->set(X1, Y1, clr2, tu, tv + fTCHeight);				v++;
						v->set(X2, Y2_, clr, tu, tv);							v++;
						v->set(X3, Y3, clr2, tu + fTCWidth, tv + fTCHeight);	v++;
						v->set(X4, Y4, clr, tu + fTCWidth, tv);					v++;
					}
					X += scw * vInterval.x;
					if (IsMultibyte())
					{
